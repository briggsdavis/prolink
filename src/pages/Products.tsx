...
import { Search, Filter, ChevronRight, X, ShoppingBag, ArrowRight } from 'lucide-react';
import { FadeIn } from '../components/Animations';

type Category = 'All' | 'Caps' | 'Winter Wear' | 'Kitchenware' | 'Bags' | 'Rainwear';
...
export const Products = () => {
  ...
  const categories: Category[] = ['All', 'Caps', 'Winter Wear', 'Kitchenware', 'Bags', 'Rainwear'];

  // Mock products
  const products = Array.from({ length: 50 }).map((_, i) => ({
    id: i,
    name: `${i % 5 === 0 ? 'Signature' : 'Premium'} ${i % 3 === 0 ? 'Baseball' : 'Snapback'} Cap v${i + 1}`,
    category: i < 20 ? 'Caps' : i < 30 ? 'Winter Wear' : i < 40 ? 'Kitchenware' : i < 45 ? 'Bags' : 'Rainwear',
    image: `https://picsum.photos/seed/prod${i}/600/600`,
    colors: ['#345eb3', '#b61011', '#608be1', '#f8f9fa'],
    description: 'A high-fidelity prototype product showing the quality and craftsmanship of Hebei Prolink.',
      ...
      <section className="bg-white border-b py-20">
        <div className="max-w-7xl mx-auto px-6">
          <div className="flex flex-col md:flex-row justify-between items-end gap-8">
            <div>
              <span className="text-primary-blue font-bold uppercase tracking-[0.3em] text-sm mb-4 block">Our Catalog</span>
              <h1 className="text-5xl md:text-6xl font-bold text-slate-900 tracking-tight leading-tight">Premium <br />Collection</h1>
            </div>
          ...
          <div className="sticky top-32">
            ...
            <div className="space-y-2">
              {categories.map((cat) => (
                ...
                >
                  {cat}
                  <ChevronRight size={16} className={`transition-transform ${activeCategory === cat ? 'translate-x-1' : 'opacity-0 group-hover:opacity-100'}`} />
                </button>
              ))}
            </div>
            ...
            <div className="mt-12 p-10 bg-white border border-slate-100 relative overflow-hidden">
              ...
              <h4 className="text-xl font-bold mb-4 relative z-10 tracking-tight text-slate-900">Custom Orders?</h4>
              <p className="text-slate-500 text-sm mb-8 relative z-10 leading-relaxed">We provide tailored manufacturing solutions for your specific brand requirements.</p>
            ...
            <AnimatePresence mode="popLayout">
              {filteredProducts.map((product, i) => (
                ...
                >
                  ...
                  <span className="text-primary-blue text-[10px] font-bold uppercase tracking-[0.2em] mb-2 block">{product.category}</span>
                  <h3 className="text-xl font-bold text-slate-900 group-hover:text-primary-blue transition-colors tracking-tight">{product.name}</h3>
                </motion.div>
              ))}
            </AnimatePresence>
          </div>
            ...
            >
              ...
              <div className="w-full md:w-1/2 p-16 flex flex-col">
                <div className="flex-grow">
                  <span className="text-primary-blue font-bold uppercase tracking-[0.3em] text-xs mb-6 block">{selectedProduct.category}</span>
                  <h2 className="text-4xl font-bold text-slate-900 mb-10 tracking-tight">{selectedProduct.name}</h2>
                  
                  <div className="mb-12">
                    <h4 className="text-xs font-bold text-slate-900 uppercase tracking-[0.2em] mb-6">Color Variants</h4>
                  ...
                  <div className="mb-12">
                    <h4 className="text-xs font-bold text-slate-900 uppercase tracking-[0.2em] mb-4">Description</h4>
                  ...
                  <div className="bg-light-grey p-8 border border-slate-100">
                    <h4 className="text-xs font-bold text-slate-900 uppercase tracking-[0.2em] mb-4">Technical Details</h4>
                    <p className="text-slate-500 text-sm leading-relaxed">{selectedProduct.technical}</p>
                  </div>
                </div>
